...
BOT_SCOPE = %w[app_mentions read,channels history,channels read chat:write chat:write.customize commands files:write groups:history groups:read im:history im:read mpim:read users.profile:read users:read incoming-webhook]
...
class Auth < Sinatra::Base
  ...
  add_to_slack_button = %(
    <a href=\"https://slack.com/oauth/authorize?client_id=#{SLACK_CONFIG[:slack_client_id]}&redirect_uri=#{SLACK_CONFIG[:redirect_uri]}&scope=#{BOT_SCOPE.join ','}\">
